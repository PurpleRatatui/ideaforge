  ...
  return (
    <div className="site-shell">
      ...
      <header className="site-header wrap">
        <a className="wordmark" href="/en/roaster" aria-label="Idea Roaster home"><span className="brand-icon"><Flame size={23} fill="currentColor" /></span>idea<span className="brand-light">forge</span><span className="brand-period">.</span></a>
        <nav aria-label="Main navigation"><a href="/en">She Is Solana</a><a href="#how-it-works">How it works</a><button onClick={showExample} disabled={loading}>See an example <ArrowRight size={15}/></button></nav>
      </header>
      <main>
        <section className="hero wrap" aria-labelledby="headline">
          <div className="hero-copy">
            <div className="eyebrow"><span className="tiny-line"/> FOR HACKATHON IDEAS THAT NEED PRESSURE</div>
            <h1 id="headline">Your idea.<br/>Under pressure.<br/><span>Ready to submit.</span></h1>
            <p className="hero-description">Before you submit to Colosseum, stress-test the idea. Get a hard score, rebuild the weak parts, and leave with a 30-day go-to-market plan.</p>
            <div className="engine-line"><span>THE WORKFLOW</span><div><Flame size={16}/> KillMyIdea <span className="engine-plus">+</span><Hexagon size={17}/> Hivemind</div></div>
            <div className="reference-collage" aria-hidden="true">
              ...
              <div className="collage-ribbon">ROAST / REBUILD / GTM</div>
              <div className="collage-badge">01/03</div>
              <div className="collage-mark">FORGE</div>
            </div>
          </div>
          <div className="input-area">
            <div className="card-note"><span>Big idea. Bring receipts.</span><ArrowDown size={22}/></div>
            <form className="idea-card" onSubmit={e => { e.preventDefault(); void roast(idea); }} aria-busy={loading}>
              <div className="card-heading"><span className="mono">01 / IDEA STRESS TEST</span><Flame size={20}/></div>
              <label htmlFor="idea">What are you building?</label>
              <p className="input-help" id="idea-help">Who is it for? What problem hurts enough?<br/>Why would someone use it or pay?</p>
              <Textarea ref={inputRef} id="idea" value={idea} onChange={e => { setIdea(e.target.value); if (error) setError(""); }} placeholder="A product for [specific user] who struggles with [real problem], using Solana because [reason it matters]…" maxLength={5000} disabled={loading} aria-describedby={`idea-help${error ? " idea-error" : ""}`} aria-invalid={!!error} className="idea-input"/>
              <div className="input-caption"><span>A rough pitch is enough. Be specific.</span><span>{idea.length.toLocaleString()} / 5,000</span></div>
              {error && <p className="form-error" id="idea-error" role="alert">{error}</p>}
              <Button type="submit" className="roast-button" disabled={loading}>{loading ? <><LoaderCircle className="spin"/> Putting it under pressure…</> : <><Flame size={19}/> Roast the idea <ArrowRight className="button-arrow" size={20}/></>}</Button>
              <p className="card-footnote">Roast first. Hivemind helps rebuild what survives.</p>
            </form>
            <button className="example-link" onClick={showExample} disabled={loading}>Need a sample? <span>Open the example</span><ArrowRight size={16}/></button>
          </div>
        </section>
        <section className="process-section wrap" id="how-it-works" aria-labelledby="process-title">
          <div className="process-intro"><h2 id="process-title">Less guessing.<br/>Better submission.</h2><span className="mono">FROM IDEA TO NEXT TEST</span></div>
          <div className="process-grid">
            <article className="process-step"><div className="step-top"><span className="step-number">01</span><Flame size={21}/></div><h3>Roast it.</h3><p>Eight hard questions. One verdict. See what is weak before the hackathon clock eats your week.</p><span className="step-provider">KILLMYIDEA</span></article>
            <article className="process-step"><div className="step-top"><span className="step-number">02</span><Sparkles size={21}/></div><h3>Rebuild it.</h3><p>Send the roast to Hivemind. Tighten the user, the angle, and the first version you can actually ship.</p><span className="step-provider">HIVEMIND</span></article>
            <article className="process-step"><div className="step-top"><span className="step-number">03</span><Target size={21}/></div><h3>Take it to market.</h3><p>Leave with positioning, one acquisition channel, and a 30-day plan to test demand after the submission.</p><span className="step-provider">YOUR NEXT MOVE</span></article>
          </div>
        </section>
        {result && <section className="results-section wrap" ref={resultsRef} aria-labelledby="results-title" tabIndex={-1}>
          <div className="results-heading"><div><span className="eyebrow">{isExample ? "ILLUSTRATIVE EXAMPLE · NOT A LIVE EVALUATION" : "YOUR IDEA, WITH THE WEAK SPOTS MARKED"}</span><h2 id="results-title">What holds up.</h2></div><Button variant="ghost" className="close-results" onClick={() => { clearHivemind(); setResult(null); inputRef.current?.focus(); }} aria-label="Close results"><X size={20}/></Button></div>
      ...
      <footer className="site-footer wrap"><span>Less guessing. More useful building.</span><a href="https://github.com/monteduro/killmyidea" target="_blank" rel="noreferrer">Built with KillMyIdea <ArrowRight size={14}/></a></footer>
    </div>
  );
}
function HivemindPanel({ mode, plan, loading, error, result, idea, copied, onCopy, onRetry, onNext }: { mode: "rebuild" | "launch"; plan: HivemindPlan | null; loading: boolean; error: string; result: ResultModel; idea: string; copied: boolean; onCopy: () => void; onRetry: () => void; onNext: () => void }) {
  ...
  const heading = mode === "rebuild" ? "Hivemind is tightening the idea." : "Now it has a first route to users.";
  if (loading && !answer) return <div className="handoff-panel hivemind-panel"><div className="section-label"><LoaderCircle className="spin" size={19}/> HIVEMIND IS WORKING</div><h3>{mode === "rebuild" ? "Creating the project and rebuilding the idea." : "Creating the project and drafting the GTM plan."}</h3><div className="status-steps"><span>New Hivemind project</span><span>Intel pass</span><span>Rebuild and GTM plan</span></div><p>This can take a little while: Hivemind creates a fresh project before writing the strategy.</p></div>;
  ...
  if (!plan || !answer) return <div className="handoff-panel hivemind-panel"><div className="section-label"><Hexagon size={19}/> NEXT: HIVEMIND</div><h3>Turn the roast into a better version.</h3><p>Hivemind creates a project from the roast, then uses it to tighten the idea and draft the go-to-market plan.</p><Button className="next-button" onClick={onRetry}>Start Hivemind <ArrowRight size={18}/></Button></div>;
  return <div className="hivemind-panel"><div className="section-label">{mode === "rebuild" ? <Sparkles size={19}/> : <Target size={19}/>} {label}</div><div className="project-strip"><span>Project created in Hivemind</span><strong>{plan.project.ready ? "Intel ready" : plan.project.enrichmentStatus === "failed" ? "Intel still usable" : "Intel queued"}</strong></div><h3>{heading}</h3><FormattedAnswer text={answer.response}/><Sources sources={answer.sources}/><Button className="next-button" onClick={onNext}>{mode === "rebuild" ? <>Make the GTM plan <ArrowRight size={18}/></> : <><RotateCcw size={16}/> Try another idea</>}</Button></div>;
}
...
function Sources({ sources }: { sources: HivemindAnswer["sources"] }) {
  ...
  return <div className="source-list"><span className="mono">SOURCES USED BY HIVEMIND</span>{sources.map((source) => <p key={`${source.title}-${source.author || ""}`}>{source.title}{source.author ? <span> by {source.author}</span> : null}</p>)}</div>;
}
